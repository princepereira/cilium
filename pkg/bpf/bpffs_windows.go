...
// The BPF filesystem (bpffs) is a Linux-only construct. On other platforms the
// paths below are only used to derive map identifiers; no filesystem is
// actually mounted, so mount-related operations are no-ops.

var (
	// Path to where bpffs is (nominally) mounted.
...
// CheckOrMountFS is a no-op on platforms without a BPF filesystem.
