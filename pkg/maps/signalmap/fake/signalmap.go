...
func (r *fakePerfReader) Read() (signalmap.Record, error) {
	paused := false

	for {
		select {
		case <-r.closed:
			return signalmap.Record{}, os.ErrClosed
		case paused = <-r.pause:
			if paused {
				continue
			}
		...
		if r.index < len(r.messages) {
			return signalmap.Record{RawSample: r.messages[r.index]}, nil
		} else {
			return signalmap.Record{LostSamples: 1}, nil
		}
	}
}
